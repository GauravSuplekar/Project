...
public class CandidateService {
    ...
    public String verify(User user) {
        Candidate candidate = candidateRepository.findByUsername(user.getUsername());
        if (candidate == null) {
            throw new org.springframework.security.core.userdetails.UsernameNotFoundException(
                    "Candidate not found with username: " + user.getUsername());
        }
    ...
    public List<ClientRequirement> getAppliedRequirements(String username) {
        if (username == null || username.isBlank()) {
            log.warn("getAppliedRequirements called with null or empty username");
            return new ArrayList<>();
        }
        log.info("Fetching candidate with username : {}", username);
        Candidate candidate = candidateRepository.findByUsername(username);
        if (candidate == null) {
            log.warn("No candidate found for username: {}", username);
            return new ArrayList<>();
        }
