...
public class AuthController {
    ...
    public ResponseEntity<?> loginCandidate(
            @RequestBody User user
    ){
        try {
            return ResponseEntity.ok(new JwtResponse(candidateService.verify(user)));
        } catch (org.springframework.security.authentication.BadCredentialsException | org.springframework.security.core.userdetails.UsernameNotFoundException ex) {
    ...
    public ResponseEntity<?> loginClient(
        ...
        try {
            return ResponseEntity.ok(new JwtResponse(clientService.verify(user)));
        } catch (org.springframework.security.authentication.BadCredentialsException | org.springframework.security.core.userdetails.UsernameNotFoundException ex) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ex.getMessage());
        }
    }
    ...
    public ResponseEntity<?> loginAdmin(
        ...
        try {
            return ResponseEntity.ok(new JwtResponse(adminService.verify(user)));
        } catch (org.springframework.security.authentication.BadCredentialsException | org.springframework.security.core.userdetails.UsernameNotFoundException ex) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ex.getMessage());
        }
    }
